...
def build_smap_url(t, version="8", minor_version="011"):
    """
    Build URL to SMAP data

    Args:
        t (datetime): The date and time for the data
        version (str): Major version number (default '8')
        minor_version (str): Minor version number (default '011')
    ...
    url = (
        ...
        "nsidc-cumulus-prod-protected/SMAP/SPL4SMGP/"
        f"{version.zfill(3)}/{t.year}/{t.month:02}/{t.day:02}/{filename}"
    )
    return url
...
def get_smap(
    start_time,
    variables=["sm_profile_wetness", "snow_mass"],
    load=True,
    version="7",
    minor_version="030",
    ...
    if file_path is None:
        logging.warning(
            "SMAP data is not available for the specified time and version."
        )
        for i in range(1, 17):  # Look back 2 more days
            earlier_time = start_time - pd.Timedelta(hours=i * 3)
            url = build_smap_url(earlier_time, version, minor_version)
            file_path = download_smap(url)
...
def build_GEOS_url(run_time=None, mode="fcast"):
    ...
    if mode == "fcast":
        if run_time is None:
            raise ValueError(
                "GEOS FP model run time must be specified for forecast mode"
            )
        return (
            f"https://opendap.nccs.nasa.gov/dods/GEOS-5/fp/0.25_deg/fcast/"
...
def add_metadata(data_set: xr.Dataset, run_mode="nrt") -> xr.Dataset:
    ...
    data_set.attrs["source"] = "LHASA V2.1.1"
    ...
    data_set.attrs["comment"] = (
        "LHASA identifies where landslides are most probable in near real time."
    )
    data_set.attrs["Conventions"] = "CF-1.8"
    data_set.attrs["ShortName"] = "LHASA"
    data_set.attrs["LongName"] = (
        "Landslide Hazard Analysis for Situational Awareness (LHASA)"
    )
    data_set.attrs["VersionID"] = "2.1.1"
    ...
    end_date = (
        (data_set["time"] + pd.Timedelta(days=1)).dt.strftime("%Y-%m-%d").values[0]
    )
...
def expose(
    ...
    variables = variables.drop_vars(
        ["time", "lat", "lon", "road_length", "population"]
    ).squeeze()
    return variables
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="LHASA 2.1.1 global landslide forecast"
    )
    parser.add_argument(
        "-v", "--version", action="version", version="LHASA version 2.1.1"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-N", "--north", type=float, default=60.0, help="maximum latitude (WGS84)"
    )
    parser.add_argument(
        "-S", "--south", type=float, default=-60.0, help="minimum latitude (WGS84)"
    )
    parser.add_argument(
        "-E", "--east", type=float, default=180.0, help="maximum longitude (WGS84)"
    )
    parser.add_argument(
        "-W", "--west", type=float, default=-180.0, help="minimum longitude (WGS84)"
    )
    parser.add_argument(
        "-sv",
        "--smap_version",
    ...
    logging.basicConfig(
        level=logging.INFO, format="%(asctime)s %(message)s", datefmt="%H:%M"
    )
    logging.info("logging started")

    if args.west > args.east:
        raise ValueError(
            f"West longitude ({args.west}) cannot be greater than East longitude ({args.east})"
        )
    ...
    static_files = [
        os.path.join(path, "static", "pga.nc4"),
    ...
    if args.exposure:
        ...
        constant_totals["road_length"] = np.maximum(
            constant_totals["road_length"], 1e-10
        )
        constant_totals["population"] = np.maximum(constant_totals["population"], 1e-10)
        admin_names = pd.read_csv(
            os.path.join(path, "exposure", "admin_names.csv"), index_col="gadm_fid"
        )
    ...
    if args.lead > 0:
        ...
        for geos_run in reversed(geos_runs):
            geos_run = geos_run.sel(
                time=slice(forecast_start_time, precipitation_end_date)
            )
            new_hours = geos_run["time"].loc[~geos_run["time"].isin(processed_hours)]
            if new_hours.size > 0:
                selected_geos_runs.append(geos_run.sel(time=new_hours))
                processed_hours = processed_hours.combine_first(new_hours).dropna(
                    "time"
                )
        ...
        geos_daily = (
            geos.resample(
                {"time": "24H"}, offset=pd.Timedelta(f"{forecast_start_time.hour}h")
            )
            .sum()
            .load()
        )
    ...
    for i, d in enumerate(daily_rain[2 : (3 + args.lead)]):
        ...
        if "nc" in args.format:
            ...
            nc_path = (
                f"{output_path}/{run_mode}/hazard/"
                f"{initialization}{date_string}.{file_ext}"
            )
            if os.path.exists(nc_path) and not args.overwrite:
                raise FileExistsError(
                    f"{nc_path} exists. Use the --overwrite argument to overwrite it."
                )
        else:
            nc_path = ""
        if "tif" in args.format:
            tif_path = (
                f"{output_path}/{run_mode}/hazard/tif/{initialization}{date_string}.tif"
            )
            if os.path.exists(tif_path) and not args.overwrite:
                raise FileExistsError(
                    f"{tif_path} exists. Use the --overwrite argument to overwrite it."
                )
        else:
            tif_path = ""
        ...
        p_landslide = fill_array(
            prediction, static_variables["land_mask"] > 0, dates[i]
        )
        p_landslide = p_landslide.where(
            static_variables["Slope"] > args.slope_threshold
        )
        if args.small:
            warnings.warn(
                (
        ...
        if args.exposure:
            csv_path = os.path.join(
                ...
                "exposure",
                "csv",
                f"{initialization}{date_string}.csv",
            )
